...
class DatabaseService:
    def __init__(self):
        try:
            ...
            self.dashboard_collection = self.db["ca_dashboard"]
            self._create_indexes()
            print("✓ MongoDB connected successfully")
            print("✓ Collections: processing_jobs, ca_dashboard")
    ...
    # ========================================================================
    # CA_DASHBOARD Collection Methods
    # ========================================================================

    def get_dashboard_document(self, dashboard_id: str) -> Optional[Dict[str, Any]]:
        """Get a document from ca_dashboard collection by _id."""
        try:
    ...
    def update_dashboard_with_urls(
        ...
        """
        Update ca_dashboard document with generated content URLs.
        
        Args:
            dashboard_id: MongoDB _id from ca_dashboard
        ...
        try:
            ...
            if result:
                print(f"✓ Updated ca_dashboard document {dashboard_id}")
                return True
            else:
                print(f"✗ Dashboard document {dashboard_id} not found")
                return False
    ...
    def get_dashboard_by_chapter(self, chapter: str) -> list:
        """Get all dashboard documents for a specific chapter."""
        try:
            docs = list(
                self.dashboard_collection.find({"chapter": chapter})
                .sort("created_at", -1)
            )
    ...
    def search_dashboard(self, query: str) -> list:
        """Search ca_dashboard documents by title or chapter."""
        try:
            docs = list(

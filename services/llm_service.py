...
#         lang  = (
#             f"Hinglish — Hindi + English mix. Technical terms ({terms}) stay in English."
#             if style == "hinglish"
#             else f"Clear simple English. Technical terms ({terms}) used precisely."
#         )
#         return f"""You are writing an educational video script for {full} students.

# HARD STRUCTURE LIMIT — MUST FOLLOW EXACTLY:
# - Write EXACTLY 30 dialogue exchanges total: 15 lines for A (Raj) + 15 lines for B (Priya).
# - Each B line: maximum 40 words.
# - Each A line: maximum 20 words.
# - Total word count must NOT exceed 1500 words.
# - After your 30th exchange, write the Yaad Rakho section (5 bullet points, max 10 words each) then STOP.
# - Do not write a 31st exchange under any circumstance.

# CHARACTERS:
# A: Raj — {full} student. Asks one focused question per line about what is on the current PDF page.
# B: Priya — {full} teacher/instructor. Answers with precise definitions, formula steps, or exam tips.
#    Priya must reference the PDF page explicitly: "Is page pe dekho...", "Yahan formula hai..."

# LANGUAGE: {lang}
# BANNED: "Good luck", "Great question", "Excellent", "Wow", "Bilkul sahi", "Bahut accha", "Of course", any filler.

# CONTENT PRIORITY (cover in this order within the 30 exchanges):
# 1. What is the topic — one-line definition (2 exchanges)
# 2. Core formula or rule — state it + one worked numerical step (6 exchanges)
# 3. Main example from the PDF — walk through numbers on screen (8 exchanges)
# 4. Key exam mistake to avoid (4 exchanges)
# 5. Exam tip — how ICAI/ICSI asks this (4 exchanges)
# 6. Yaad Rakho — 5 bullet takeaways (last section, not an exchange)

# PDF PAGE TAGS — required for video sync:
# Insert a tag line on its OWN LINE before each new topic. Format exactly:
# [PAGE N] [TOPIC: Topic Name]
# Insert at least one tag per 5 exchanges. Never put tags inside A: or B: lines.
# Skip TOC, index, and cover pages — start from first content page.

# FORMAT (no blank lines between exchanges):
# [PAGE 1] [TOPIC: Introduction]
# A: [question max 20 words]
# B: [answer max 40 words]
# A: [follow-up max 20 words]
# B: [answer max 40 words]
# [PAGE 2] [TOPIC: Core Formula]
# A: [question]
# B: [answer]
# ...continue until exactly 30 exchanges...
# Yaad Rakho:
# • [point 1]
# • [point 2]
# • [point 3]
...
#         """
#         Generate a 7-15 min conversational transcript from the
...
# {simplified_text[:10000]}
# --- SIMPLIFIED PDF CONTENT END ---

# Now write the complete 7-15 minute dialogue script covering every concept above.
# """

...
class ContentProcessingOrchestrator:
    ...
    async def process(
        ...
        reads the existing simplified_pdf_url from the ca_dashboard document
        ...
        Args:
            ...
            dashboard_id: MongoDB _id from ca_dashboard collection
        ...
        try:
            ...
            # ----------------------------------------------------------------
            # Step 1: Read simplified_pdf_url from ca_dashboard
            #         The client must upload the simplified PDF before triggering
            #         this pipeline. If the URL is absent, abort early.
            # ----------------------------------------------------------------
            print(f"[Job {job_id}] Fetching dashboard record {dashboard_id}...")
            ...
            if dashboard_id:
                ...
                if success:
                    print(f"[Job {job_id}] ✓ ca_dashboard {dashboard_id} updated successfully")
                else:
                    print(f"[Job {job_id}] ✗ Failed to update ca_dashboard {dashboard_id}")
